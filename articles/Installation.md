...
If you do not have a suitable JDK installed, you can download and
install it from various vendors such as the [Adoptium Working
Group](https://adoptium.net/de/temurin/releases) or
...
1.  Windows users should activate the update of “JavaSoft registry keys”
    ...
    package to find the SDK installation. Depending on the SDK vendor
    you choose, the option might be named differently.
...
The error message `JAVA_HOME cannot be determined from the Registry`
indicates that the `rJava` package cannot find the JDK installation.
This can cause further errors such as
`ERROR: lazy loading failed for package 'FastRet'` when trying to load
FastRet. See [rJava installation fails](#rjava-installation-fails) for
possible solutions.

[![R console error message showing 'JAVA_HOME cannot be determined from
the Registry' indicating rJava cannot find Java
installation](Installation/error_JAVA_HOME_not_found.png)](https://spang-lab.github.io/FastRet/articles/Installation/error_JAVA_HOME_not_found.png)

### rJava installation fails

If the installation of the `rJava` package fails:

1.  Make sure you have a suitable JDK installed on your system, as
    described in [Install Java Development Kit
