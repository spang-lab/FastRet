...
- match_rts:
  ...
  Logical indicating how to obtain the RT feature used during training
  of the adjustment model. If TRUE (default), RT is obtained by matching
  rows in `new_data` to rows in `frm$df` based on `match_keys`. If
  FALSE, RT is obtained by applying the base model to `new_data`.
...
generated by calling `predict(frm, new_data, adjust = FALSE)`. If
