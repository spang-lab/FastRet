...
#' @description Changes [pkgload::load_all()] so that it doesn't lock the environment anymore. This allows to use [devtools::load_all()] with option `reset = FALSE`, which is less accurate but much faster.
#' @noRd
...
patch_shiny <- function() {
    # Fixes shiny autoreload features and suppresses warnings when sourcing an R package as shiny app.
...
cachedFuncWithFilePatched <- function(dir,
    ...
    # message("Entered patched version of cachedFuncWithFile")
