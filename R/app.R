...
#' @details If you set `nw = 3` and `nsw = 4`, you should have at least 16 cores, one core for the shiny main process. Three cores for the three worker processes and 12 cores (3 * 4) for the subworkers. For the default case, `nworkers = 2` and `nsw = 1`, you only need 3 cores, as `nsw = 1` means that all subprocesses will run sequentially.
#' @keywords public
#' @examples
...
#' @description Creates the FastRet GUI
#' @param port The port the application should listen on
#' @param host The address the application should listen on
#' @param reload Whether to reload the application when the source code changes
