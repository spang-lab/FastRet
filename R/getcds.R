...
getCDs <- function(df, verbose = 1, nw = 1) {
  ...
  if (verbose >= 2) {
    ...
    suppressWarnings <- function(expr) expr # Don't suppress warnings later on
  }

  # Return pregenerated results if mocking is on (e.g. while testing the GUI)
  ...
  fmt <- "Obtaining chemical descriptors for a dataframe of dimension %d x %d"
  catf(fmt, nrow(df), ncol(df))
  verbose01 <- if (verbose == 0) 0 else 1
  a <- Sys.time()
  if (nw > 1) {
    catf("Creating cluster for parallel processing")
    # Parallelization using forking is not possible. Probably because the
    # `getCDsFor1Molecule` calls `rcdk::eval.desc` which calls java in the
    # background and the java interface cannot be duplicated.
    ...
    cds <- parLapply(cl, df$SMILES, getCDsFor1Molecule, verbose = verbose01)
  } else {
    catf("Calculating chemical descriptors")
    cds <- lapply(df$SMILES, getCDsFor1Molecule, verbose = verbose01)
  }
...
#' `~/.cache/FastRet/getCDsFor1Molecule/` to speed up subsequent calls.
...
getCDsFor1Molecule <- function(smi = "O=C(O)CCCCCCCCCO",
                               cache = TRUE,
                               verbose = 1) {
  ...
  if (cache) {
    ...
    # To understand the following part, a rough understanding of rcdk is
    # required. Therefore, it is recommended to read the following vignette:
    ...
    rcdk::convert.implicit.to.explicit(obj) # Add hydrogen atoms if omitted in SMILES
...
#' @description
#' Vectors containing the feature names of the chemical descriptors listed in
#' [CDNames].
#' @keywords internal
#' @seealso [CDNames]
...
#' - `CDs`: A data frame. The column names of `CDs` are the chemical descriptors
#'   listed in [CDFeatures]. The rownames in `CDs` are SMILES strings.
#' - `CDRowNr`: A list. The names of the list elements equal the rownames of
#' - `CDs`. The values are the indices of the rows in the `CDs` data frame.
#' @references
...
make_cachedata_cds <- function() {
    ...
    # Use nw=1 to ensure that the RAM is populated in this process and not in a
    # worker processes
...
#' @description Names of all chemical descriptors available in rCDK v2.3.
...
#' @description Names of all chemical descriptors available in rCDK v2.9.
